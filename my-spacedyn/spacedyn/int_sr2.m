...
%
%		INT_SR2 returns velocity & position vectors 
...
%
% INT_SR    采用辛普森规则对系统运动进行积分
%
%        INT_SR2 返回系统各连杆在惯性坐标系中的速度与位置向量
%        采用以下简化辛普森规则：
%
%       f(t+dt) = f(t) + ( dt/6 ) * [ g(t) + 4*g(t+dt/2) + g(t+dt)]
%       其中 df(t)/dt = g(t)
%
%       g(t+dt/2) 和 g(t+dt) 在函数内部更新。
%
%        (采用罗德里格斯公式处理无限小旋转
%        实现从A0到A0_n的更新。
%        该方法在实际应用中优于公式
...
%        [R0,A0,v0,w0,q,qd]=int_sr(R0,A0,v0,w0,vd0,wd0,q,qd,qdd)
%    
%        注：v0,w0,vd0,wd0 位于惯性坐标系。


function  SV = int_sr2( LP , SV )
